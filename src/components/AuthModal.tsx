...
export const AuthModal: React.FC<AuthModalProps> = ({ onSuccessNavigate }) => {
  ...
  return (
    ...
    >
      <div
        id="auth-modal-card"
        className="relative w-full max-w-md bg-[#06110B] border border-[#183C2A] shadow-2xl p-6 sm:p-8 rounded-xs text-[#FAF9F5]"
        onClick={e => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 p-1.5 text-[#9DB4A7] hover:text-white transition-colors"
          aria-label="Close"
        >
          <X size={18} />
        </button>
            ...
            <form onSubmit={handleGoogleSubmit} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="w-1/3 py-2.5 border border-[#183C2A] text-xs text-white hover:bg-[#132A1E] rounded-xs transition-colors"
                >
                  Back
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 py-2.5 bg-[#D4AF37] hover:bg-[#E5C583] text-[#06110B] text-xs uppercase tracking-wider font-bold disabled:opacity-60 rounded-xs transition-colors shadow-md"
                >
          ...
          <div>
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="font-editorial text-2xl sm:text-3xl text-white">
                {mode === 'signin' ? 'Sign In to Atelier' : 'Create Your Atelier Profile'}
              </h2>
              <p className="text-xs text-[#9DB4A7] mt-1">
                {mode === 'signin'
                  ? 'Access your orders, wardrobe archive, and concierge preferences.'
                  : 'Register for private allocations, bespoke fittings, and order tracking.'}
              </p>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 px-4 bg-[#0A1C14] hover:bg-[#132A1E] text-white border border-[#183C2A] flex items-center justify-center gap-3 transition-colors text-xs font-sans font-medium mb-4 rounded-xs"
            >
            ...
            <div className="flex items-center my-4">
              ...
              <span className="px-3 text-[11px] text-[#9DB4A7] uppercase tracking-wider font-mono-luxury">
                or with email
              </span>
              <div className="flex-1 border-t border-[#183C2A]" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-[#D4AF37] hover:bg-[#E5C583] text-[#06110B] text-xs uppercase tracking-wider font-bold transition-colors disabled:opacity-60 mt-2 rounded-xs shadow-md"
              >
              ...
              {mode === 'signin' ? (
                <p>
                  ...
                  <button
                    ...
                    className="text-[#E5C583] font-medium underline hover:text-white ml-1"
                  >
                    Register here
                  </button>
                </p>
